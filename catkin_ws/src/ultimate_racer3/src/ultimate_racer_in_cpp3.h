...
private:
  ...
  // Stats
  int deque_len = 5;
  std::deque<float> prev_best_scan;
  std::deque<float> prev_best_idx;
  double mean_best_scan;
  double mean_best_idx;

  void calc_stats(int best_idx, float best_scan);
  void add_to_deque(std::deque<float> & q, float el);
  float add_and_calc_mean(std::deque<float> & q, float el);
